...
export default function Grid({ guesses, currentGuess, answer, maxGuesses, wordLength, shake }: GridProps) {
  ...
  return (
    <div className="grid gap-1.5 sm:gap-2 mb-6 sm:mb-8 px-2">
      {/* Completed guesses */}
      {guesses.map((guess, i) => (
      ...
      {/* Current guess */}
      {currentGuess && (
        <div className={`flex gap-1.5 sm:gap-2 justify-center ${shake ? 'shake' : ''}`}>
      ...
      {/* Empty rows */}
      {empRows.map((_, i) => (
        <div key={`empty-row-${i}`} className="flex gap-1.5 sm:gap-2 justify-center">
